...
class Graph:
    ...
    def has_cycle(self):
        ...
        def dfs(vertex_index, visited, recursion_stack):
        
            visited[vertex_index] = True
            recursion_stack[vertex_index] = True
            ...
            recursion_stack[vertex_index] = False
            return False
    ...
    def get_registration_plan(self):
        ...
        num_vert = len(temp_vertices)
        prereq_courses = [0] *num_vert
        ...
        for i in range(num_vert):
            if prereq_courses[i] == 0:
                queue.enqueue(i)
        while not queue.is_empty():
            curr_semester = []

            for _ in range(min(4,queue.size())):
                index_of_course = queue.dequeue()
                curr_semester.append(temp_vertices[index_of_course].label)

                for neighbor_index in range(num_vert):
                    if temp_matrix[index_of_course][neighbor_index] == 1:
                        temp_matrix[index_of_course][neighbor_index] = 0
                        prereq_courses[neighbor_index] -= 1

                        if prereq_courses[neighbor_index] == 0:
...
def main():
    ...
    num_verts = int(lines[0].strip())
   
    i = 1
    while i <= num_verts:
        vert_lab = lines[i].strip()
        graph.add_vertex(vert_lab)
        i+= 1

    num_edge = int(lines[i].strip())
    i+=1

    count_e = 0
    while count_e < num_edge:
        edge = lines[i].strip().split()
        course_from_index = graph.get_index(edge[0])
        course_to_index = graph.get_index(edge[1])
        graph.add_edge(course_from_index, course_to_index)
        count_e +=1
        i+= 1
